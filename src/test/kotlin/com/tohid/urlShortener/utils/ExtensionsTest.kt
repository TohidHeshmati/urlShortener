...
class ExtensionsTest {
    ...
    fun `returns at least 8-character strings with valid base62 characters`(value: Long) {
    ...
    fun `is longer than 8 characters for large numbers`(value: Long) {
        val base62 = value.toBase62()
        assertTrue(base62.length > 8, "Expected longer than 8 characters for large numbers")
    }

    @Test
    fun `of zero returns correct padded value`() {
    ...
    @Test
    fun `produces unique output for different inputs`() {
